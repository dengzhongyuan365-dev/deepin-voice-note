...
resolve_app_bin()
{
    if [[ -n "${DEEPIN_VOICE_NOTE_BIN:-}" ]]; then
        printf '%s\n' "${DEEPIN_VOICE_NOTE_BIN}"
        return 0
    fi
    ...
    local candidates=(
        "${PROJECT_ROOT}/build/src/${APP_NAME}"
        "${PROJECT_ROOT}/src/${APP_NAME}"
    )
    local candidate
    ...
    echo "${APP_NAME} executable not found; set DEEPIN_VOICE_NOTE_BIN or build the project first" >&2
    return 127
}
